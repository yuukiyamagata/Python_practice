...
Pythonでオブジェクトを作成した場合idという値が振られる
id()関数によって確認が可能
...
# クラス変数とメソッド
インスタンス変数...個々のインスタンスに紐づく変数
クラス変数...そのクラスの中でのみ認識される変数。
              すべてのインスタンスで共有される。クラスに紐づくデータのこと
・クラス変数はクラス内のメソッドから「クラス名.クラス変数名」で使用可能
メソッド...インスタンス変数名.メソッド名()で呼び出せる。インスタンス内ではself.メソッド名で呼び出せる
...
# クラスの初期化
def __init__(self, 引数1)を使う
init...初期化するという意味の英単語initializeから来ている
self...自分自身。クラスから生成したインスタンス自身を指す
*selfはレシーバーとも呼ばれる
__init__()はPython独自のメソッド
クラスからインスタンスが生成される時に呼び出される
この初期化メソッドでインスタンス定数を定義することも可能
self.インスタンス定数名 = 引数1のように定義

*このクラス(もの)はどんなデータを持っているべきだろうか、どんなものを持っていればそのものらしくなるだろうかと考える

# 継承とは
・他クラスの属性やメソッドを「受け継ぐ」こと
・引き継がれるクラスを親クラス(スーパークラス)、引き継ぐクラスを子クラス(サブクラス)などと呼ぶ
・子クラスは親クラスを専門特価させたものであり、子は親に含まれる
例:「犬」は「動物」であり、「犬」クラスは「動物」クラスを継承する
「犬」は「動物」の属性やメソッドを使うことができる

# オーバーライドとsuperを使った親への要請
・overreide...「優先する」「覆す」などの意味(オーバーライド)
プログラミングの中では「上書きする」の意味に近い
-> 親から継承したメソッドを書き換えることを「オーバーライド」という

・super...親(スーパー)クラスを指す
親クラスの変数やメソッドを参照しつつ、子クラス独自の処理を追加したいときなどに使う
